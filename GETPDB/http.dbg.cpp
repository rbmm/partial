...
namespace CDnsCache
{
	enum { entry_count = 0x400 };
	ULONG _crc[entry_count];
	ULONG _ip[entry_count];
	LONG _i;

	void Add(ULONG ip, PCSTR host)
	{
		ULONG crc = RtlComputeCrc32(0, host, (ULONG)strlen(host));
		if (!findDWORD(entry_count, _crc, crc))
		{
			LONG i = (InterlockedIncrement(&_i) - 1) & (entry_count - 1);
			_ip[i] = ip;
			// release
	...
	ULONG get(PCSTR host)
	{
		ULONG crc = RtlComputeCrc32(0, host, (ULONG)strlen(host));
		if (PULONG pu = findDWORD(entry_count, _crc, crc))
		{
			DbgPrint("get(%s)\n", host);
			// asquire
			return *(PULONG)RtlOffsetToPointer(_ip, RtlPointerToOffset(_crc, pu));
		}
...
public:
	...
	void Write(PFILE_SEGMENT_ELEMENT SegmentArray, ULONG Length)
	{
		...
		{
			_pDD->WriteBegin();
			NTSTATUS status = NtWriteFileGather(m_hFile, 0, 0, irp, irp, SegmentArray, Length, &_ByteOffset, 0);
